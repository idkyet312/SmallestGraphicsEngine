...
LevelValidationResult ValidateLevel(const LevelDefinition& level) {
    ...
    if (!std::isfinite(level.terrainIslandScaleX) ||
        level.terrainIslandScaleX < 0.5f || level.terrainIslandScaleX > 5.0f ||
        !std::isfinite(level.terrainIslandScaleZ) ||
        level.terrainIslandScaleZ < 0.5f || level.terrainIslandScaleZ > 5.0f)
        result.errors.push_back("terrain island scale must be between 0.5 and 5");
