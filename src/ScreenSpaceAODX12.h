...
public:
    ...
    bool Init() {
        ...
        ComPtr<ID3DBlob> vs, ps, psMSAA, errors;
        if (!Compile(source, "VSMain", "vs_5_0", flags, vs, errors) ||
            ...
            !CreateRootSignature() || !CreatePipelines(vs.Get(), ps.Get(), psMSAA.Get()) ||
    ...
    void Render(const Scene& scene, ID3D12Resource* depthResource,
        ...
        list->SetPipelineState(hdrTarget ? pipelineHDR_.Get() :
            (multisampledDepth ? pipelineMSAA_.Get() : pipeline_.Get()));
        ...
        list->SetGraphicsRootConstantBufferView(0,
            constantBuffer_->GetGPUVirtualAddress() + g_dx12.frameIndex * 256u);
...
private:
    ...
    bool Compile(const std::string& source, const char* entry, const char* target,
                 UINT flags, ComPtr<ID3DBlob>& output, ComPtr<ID3DBlob>& errors) {
        errors.Reset();
        HRESULT hr = D3DCompile(source.data(), source.size(), "screen_space_ao.hlsl",
    ...
    bool CreateRootSignature() {
        ...
        if (FAILED(D3D12SerializeRootSignature(&desc,
                D3D_ROOT_SIGNATURE_VERSION_1, &blob, &errors))) return false;
        return SUCCEEDED(g_dx12.device->CreateRootSignature(0,
            blob->GetBufferPointer(), blob->GetBufferSize(),
            IID_PPV_ARGS(&rootSignature_)));
    }

    bool CreatePipelines(ID3DBlob* vs, ID3DBlob* ps, ID3DBlob* psMSAA) {
        ...
        if (FAILED(g_dx12.device->CreateGraphicsPipelineState(&desc,
                IID_PPV_ARGS(&pipeline_)))) return false;
        desc.RTVFormats[0] = DXGI_FORMAT_R16G16B16A16_FLOAT;
        if (FAILED(g_dx12.device->CreateGraphicsPipelineState(&desc,
                IID_PPV_ARGS(&pipelineHDR_)))) return false;
        desc.RTVFormats[0] = DXGI_FORMAT_R8G8B8A8_UNORM;
        desc.PS = { psMSAA->GetBufferPointer(), psMSAA->GetBufferSize() };
        return SUCCEEDED(g_dx12.device->CreateGraphicsPipelineState(&desc,
            IID_PPV_ARGS(&pipelineMSAA_)));
    }
    ...
    bool CreateResources() {
        ...
        if (FAILED(g_dx12.device->CreateCommittedResource(&heap,
                ...
                nullptr, IID_PPV_ARGS(&constantBuffer_)))) return false;
        return SUCCEEDED(constantBuffer_->Map(0, nullptr,
    ...
    void Update(const Scene& scene, ID3D12Resource* depth, bool multisampled) {
        ...
        constants.cameraNearFar = { scene.camera.Position.x, scene.camera.Position.y,
        ...
        constants.lightDirection = { lightDirection.x, lightDirection.y,
            lightDirection.z, scene.contactShadowStrength };
        constants.aoParams = { scene.ambientOcclusionRadius,
            scene.ambientOcclusionStrength, scene.ambientOcclusionBias,
            scene.cameraFar };
        ...
        g_dx12.device->CreateShaderResourceView(multisampled ? nullptr : depth,
            &srv, handle);
    ...
    ComPtr<ID3D12PipelineState> pipeline_, pipelineHDR_, pipelineMSAA_;
