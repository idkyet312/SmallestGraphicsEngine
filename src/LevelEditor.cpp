...
float PickRadius(LevelEntityType type) {
    ...
    case LevelEntityType::Fern: return 0.55f;
    default: return 0.9f;
    }
}
...
ImU32 TypeColor(LevelEntityType type, bool selected) {
    ...
    case LevelEntityType::Fern: return IM_COL32(35, 170, 75, 220);
    }
...
void LevelEditor::NewFromLevelOne() {
    ...
    dirty_ = false; runtimeDirty_ = true; foliageRuntimeDirty_ = true; playing_ = false;
...
void LevelEditor::Undo() {
    ...
    dirty_ = true; runtimeDirty_ = true; foliageRuntimeDirty_ = true;
}
...
void LevelEditor::OnKeyDown(unsigned key, bool controlDown) {
    ...
    else if (key == 'B') foliageTool_ = foliageTool_ == 1 ? 0 : 1;
...
void LevelEditor::PaintFoliage(CXMMATRIX view, CXMMATRIX projection,
    ...
    if (foliageTool_ == 0 || ImGui::GetIO().WantCaptureMouse || ImGuizmo::IsOver()) return;
    ...
    const LevelEntityType type = foliageType_ == 0 ? LevelEntityType::GrassPatch :
        (foliageType_ == 1 ? LevelEntityType::Fern : LevelEntityType::Palm);
    if (foliageTool_ == 2) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        entity.name = std::string(type == LevelEntityType::Palm ? "Tree" :
            (type == LevelEntityType::Fern ? "Fern" : "Grass Patch")) + " " +
            std::to_string(entity.id);
        ...
        } else if (type == LevelEntityType::Fern) {
...
LevelEditorActions LevelEditor::Render(Camera&, CXMMATRIX view,
    ...
    const LevelEntityType types[] = { LevelEntityType::WoodHouse, LevelEntityType::MetalHouse,
        ...
        LevelEntityType::GrassPatch, LevelEntityType::Fern };
    for (auto type : types) {
    ...
    ImGui::RadioButton("Select", &foliageTool_, 0);
    ImGui::SameLine();
    ImGui::RadioButton("Paint (B)", &foliageTool_, 1);
    ImGui::SameLine();
    ImGui::RadioButton("Erase", &foliageTool_, 2);
    const char* foliageTypes[] = { "Grass", "Fern", "Trees" };
    ...
    entity = Selected();
    if (entity && entity->enabled && foliageTool_ == 0) {
    ...
    if (foliageTool_ == 0) SelectFromViewport(view, projection);
    ImDrawList* draw = ImGui::GetForegroundDrawList();
    XMMATRIX viewProjection = view * projection;
