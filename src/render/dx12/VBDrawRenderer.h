#ifndef VBDRAW_RENDERER_H
#define VBDRAW_RENDERER_H
...
inline XMFLOAT4 ComputeDrawItemBounds(const VBDrawItem& item) {
...
inline void CullAndBatchDrawItems(Scene& scene, const XMMATRIX& view, const XMMATRIX& proj,
                                  std::vector<VBDrawItem>& items) {
    for (VBDrawItem& item : items)
        item.worldBounds = ComputeDrawItemBounds(item);
    items.erase(std::remove_if(items.begin(), items.end(),
        [&](const VBDrawItem& item) {
...
static_assert(offsetof(GPUVisibilityCullInput, command) == 0,
    "GPU cull command offset must match visibility_cull_cs.hlsl.");
...
static_assert(offsetof(GPUVisibilityCullInput, padding) == 84,
    "GPU cull padding offset must match visibility_cull_cs.hlsl.");
...
struct GPUDrivenVisibilityContext {
    ...
    bool WriteInput(UINT index, const GPUVisibilityCullInput& input) {
        assert(mappedInputs != nullptr);
        assert(g_dx12.frameIndex < FRAME_COUNT);
        assert(index < maxCommands);
        if (!mappedInputs || g_dx12.frameIndex >= FRAME_COUNT ||
            index >= maxCommands) return false;
        // MoveToNextFrame has already retired this slot. Never seed the other
        // slots here: their previous commands may still be in flight.
        mappedInputs[g_dx12.frameIndex * maxCommands + index] = input;
        return true;
    }
...
inline void RenderVBDraw(Scene& scene, ShaderDX12& shader,
    ...
    for (UINT i = 0; i < drawCount; i++) {
        ...
        if (indexedOpaque) {
            ...
            input.worldBounds = drawItems[i].worldBounds;
            input.cullFlags = drawItems[i].frustumOnly
            ...
            if (context.WriteInput(inputIndex, input))
                ++batchCount;
            else
