...
public:
    ...
    Model() : loaded(false), position(0.0f), rotation(0.0f), scale(1.0f), scene(nullptr) {}

    Model(const std::string& path) : loaded(false), position(0.0f), rotation(0.0f), scale(1.0f), scene(nullptr) {
        loadModel(path);
    }
    ...
    bool loadModel(const std::string& path) {
        ...
        // Store the scene pointer - we need to keep the importer alive
        // So we need to make a persistent copy of the scene
...
private:
    ...
    unsigned int loadTexture(const char* path) {
        ...
        if (filename[0] == '*') {
            int texIndex = atoi(filename.c_str() + 1);
            if (scene && texIndex >= 0 && texIndex < (int)scene->mNumTextures) {
                return loadEmbeddedTexture(scene->mTextures[texIndex], filename);
            }
        }
    ...
    unsigned int loadEmbeddedTexture(aiTexture* texture, const std::string& texName) {
        // Check if texture was already loaded
        if (texturesLoaded.find(texName) != texturesLoaded.end()) {
            return texturesLoaded[texName];
        }
        ...
        if (texture->mHeight == 0) {
            ...
            if (data) {
                ...
                stbi_image_free(data);
                texturesLoaded[texName] = textureID;
                std::cout << "Loaded embedded texture: " << texName << std::endl;
                return textureID;
            } else {
                std::cout << "Failed to load embedded texture: " << texName << std::endl;
                return 0;
            }
        } else {
            ...
            texturesLoaded[texName] = textureID;
            std::cout << "Loaded embedded texture (uncompressed): " << texName << std::endl;
            return textureID;
        }
    }
};
